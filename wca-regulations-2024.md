...
## <article-1><officials><officials> 제 1조: 운영진

- 1a) 각 대회는 WCA 파견위원과 다음과 같은 운영진(한 명 이상)을 포함해야 한다: 심판, 스크램블러, 기록 입력자.
- 1b) 대회의 운영진은 대회 전, 중 후의 실행 계획에 대한 책임이 있다.
- 1c) WCA 파견위원은 대회가 WCA 규정과 적용 가능한 WCA 정책 혹은 요구 사항들을 준수하는지 보장할 책임이 있다. WCA 파견위원은 운영진의 다른 구성원이 특정한 책무를 수행하도록 지명할 수 있으나, 책무가 어떻게 수행되는지에 대한 책임은 궁극적으로 WCA 파견위원에게 있다.
  - 1c1) WCA 파견위원은 대회가 [WCA 스크램블 책임 정책](https://documents.worldcubeassociation.org/documents/policies/external/Scramble%20Accountability.pdf)을 따르는지 보장해야 한다.
- 1e) 각 종목은 한 명 이상의 심판이 있어야 한다.
  - 1e1) 심판은 해당 종목의 절차를 진행하는 것에 대한 책임이 있다.
    - 1e1a) 한 명의 심판은 모든 WCA 규정을 항상 따른다는 것을 보장할 수 있는 한, WCA 파견위원의 재량 하에 여러 선수에 대한 심판 업무를 동시에 수행할 수 있다.
  - 1e2) 모든 선수는 심판 업무 수행이 가능해야 한다. 업무 수행을 요구 받을 시, 오직 합당한 이유(예: 퍼즐에 익숙하지 않음)로만 WCA 파견위원의 재량 하에 업무로부터 제외될 수 있다. 페널티: 대회의 한 종목, 복수의 종목 또는 전 종목에서 실격 ([규정 2k](regulations:regulation:2k) 참고).
- 1f) 각 종목은 한 명 이상의 스크램블러가 있어야 한다. 예외: 3x3x3 최소회전
  - 1f1) 스크램블러는 기록 측정을 위한 퍼즐을 준비하기 위해 스크램블 알고리즘을 적용한다. ([규정 A2](regulations:regulation:A2) 참고).
  - 1f2) 모든 선수는 스크램블러 업무 수행이 가능해야 한다. 업무 수행을 요구 받을 시, 오직 합당한 이유(예: 스크램블 회전 기호에 익숙하지 않음)로만 WCA 파견위원의 재량 하에 업무로부터 제외될 수 있다. 페널티: 대회의 한 종목, 복수의 종목 또는 전 종목에서 실격 ([규정 2k](regulations:regulation:2k) 참고).
- 1g) 각 종목은 한 명 이상의 기록 입력자가 있어야 한다.
  - 1g1) 기록 입력자는 결과를 집계하는 책임이 있다.
  - 1g2) 기록지의 결과에 대한 변경은 오직 WCA 파견위원의 재량 하에 허용될 수 있다.
- 1h) 종목의 동일한 라운드에 속한 선수는 하나의 조 혹은 복수의 조로 나누어져 참가할 수 있다.
  - 1h1) 한 라운드의 스크램블러와 심판은 해당 라운드에서 그들의 모든 시도가 완료되기 전에는 자신이 속한 조의 선수에 대하여 스크램블 업무/심판 업무를 해서는 안 된다. WCA 파견위원의 재량 하에 자신이 속한 조에 대하여 스크램블 업무/심판 업무를 할 수 있지만, 운영진은 그들이 아직 측정하지 않은 시도에 대한 스크램블 알고리즘을 볼 수 없도록 해야 한다. ([규정 4b2](regulations:regulation:4b2) 참고).
...
- 4b) 퍼즐은 컴퓨터가 생성한 임의의 스크램블 알고리즘로 스크램블되어야 한다.
  ...
  - 4b2) 한 조의 측정이 시작되기 전에는 오직 WCA 파견위원만 해당 조의 스크램블 알고리즘을 사용할 수 있어야 하며, 해당 조의 측정이 종료되기 전까지는 WCA 파견위원과 스크램블러만이 사용 가능하다. 예외: 3x3x3 최소회전 종목은 선수가 라운드 진행 도중에 스크램블 알고리즘을 받는다 ([제 E조](regulations:article:E) 참고).
    - 4b2a) 대회에 지정된 WCA 파견위원은 자신이 향후 조에 대한 스크램블 알고리즘을 일시적으로 제공하지 못 할 때만 (예: 본인의 기록 측정) 운영진의 다른 구성원에게 스크램블 알고리즘에 대한 접근 권한을 부여할 수 있으며, 이 경우 가능한 최소한의 접근만을 허용해야 한다.
...
- 9f) 라운드의 결과는 다음과 같이 측정된다:
  - 9f1) 3x3x3 눈 가리고 맞추기 (복수) 종목에서의 결과를 제외한 10분 이하로 측정된 결과는 100 분의 1초 단위로 절사된다. 측정된 10분 이하의 모든 절단 평균과 평균은 가장 가까운 100 분의 1초 단위로 반올림되어 기록된다.
  - 9f2) 3x3x3 눈 가리고 맞추기 (복수) 종목의 모든 결과를 포함한 10분을 초과하는 측정된 모든 결과, 절단 평균, 기록은 가장 가까운 초 단위로 반올림되어 기록된다 (예: X.49는 X로, X.50는 X+1로 반올림된다.).
  ...
  - 9f6) "최고(X회)" 라운드에서, 각 선수는 X회의 시도를 할당 받는다. 해당 시도들 중 최고 기록으로 선수의 라운드 순위를 결정한다.
  - 9f7) "최고(X회)" 라운드에서, DNF나 DNS는 가능한 기록 중 최저 기록이다.
  - 9f8) "평균(5회)" 라운드에서, 선수는 5회의 시도를 할당 받는다. 5회의 시도 중에서, 최고 기록과 최저 기록은 제외되며, 남은 3회의 산술 평균으로 해당 선수의 라운드 순위를 결정한다.
  ...
  - 9f10) "평균(3회)" 라운드에서, 선수는 3회의 시도를 할당 받는다. 세 시도들의 산술 평균으로 선수의 라운드 순위를 결정한다.
  - 9f11) "평균(3회)" 라운드에서, 참가자가 최소 한 개 이상의 DNF 혹은 DNS 기록을 받았다면 그 선수의 해당 라운드의 평균은 DNF이다.
  - 9f12) "최고(X회)" 라운드에서, 순위는 선수의 최고 기록을 기반으로 매겨진다. 다음 항목이 결과를 비교하는데 적용된다:
    - 9f12a) 측정된 결과에 대하여 "더 나은"은 더 짧은 시간으로 정의된다.
    - 9f12b) 3x3x3 최소회전 종목에 대하여 "더 나은"은 더 짧은 길이의 해답으로 정의된다.
    - 9f12c) 3x3x3 눈 가리고 맞추기 (복수) 종목은 맞추어진 퍼즐의 개수에서 맞추지 못한 퍼즐의 개수를 뺀 값으로 순위가 매겨지며, 그 차이가 클수록 더 나은 결과이다. 두 값의 차가 0보다 작거나 오직 한 개의 퍼즐만이 맞추어 졌다면, 해당 시도는 맞추어지지 않는 것으로 간주된다 (DNF). 선수가 동일한 결과를 얻을 경우 시도의 총 소요 시간을 기준으로 순위가 매겨지며, 해당 시간이 짧을 수록 더 나은 결과이다. 만약 선수가 동일한 결과를 동일한 총 소요 시간에 달성한 경우 맞추지 못한 퍼즐 수를 기준으로 순위가 매겨지며, 맞추지 못한 퍼즐이 적을수록 더 나은 결과이다.
  - 9f13) "평균(3회)"과 "평균(5회)" 라운드에서 순위는 선수의 절단평균/평균을 기준으로 매겨지며, 기록된 결과가 작을수록 "더 나은" 결과이다.
  - 9f14) "평균(3회)"과 "평균(5회)" 라운드에서 둘 이상의 선수가 동일한 절단평균/평균 결과를 달성한 경우 순위는 각 선수의 최고 기록으로 매겨지며, "더 나은"은 기록된 결과가 더 작은 것으로 정의된다.
  - 9f15) 동일한 라운드에서 평균과 최고 기록이 완전히 동일한 결과를 달성한 선수는 라운드에서 동일한 순위를 부여받는다.
- 9g) 컷오프 라운드는 "최고(X회)" 컷오프 형식과 컷오프 요구 사항이 (예: 컷오프 요건이 정확히 2분 미만보다 나은 결과여야 하는 "최고(2회)") 존재하는 라운드이다. 선수가 본인의 컷오프 형식 시도에서 적어도 하나의 시도가 컷오프 요건을 만족할 경우, 선수는 남은 시도를 측정할 수 있는 자격이 주어진다. 컷오프 형식에서의 시도들은 전체 라운드 형식에 포함된다.
- 9i) WCA 공인 대회의 결과는 WCA 세계 랭킹에 등재되어야 한다.
  ...
  - 9i3) 종목에 대한 WCA 규정이 변경됐을 경우 남아있는 지역 신기록은 새로운 WCA 규정 하에 경신된 신기록이 나오기 전까지 유효하다.
- 9j) 각 종목은 대회 당 최대 한 번만 개최될 수 있다.
- 9k) 모든 선수는 WCAT에 의해 특별히 승인된 상황을 제외하고는 대회의 모든 종목에 참가할 수 있다.
- 9l) 각 라운드는 동일한 종목의 후속 라운드가 시작되기 전에 완료되어야 한다. 예외: 추가 시도는 최초에는 유효하지 않았던 시도를 대체하기 위해 대회 도중 아무 때나 부여될 수 있다.
- 9m) 각 종목은 최대 4 라운드까지 진행될 수 있다.

...
- 1e) 각 종목은 반드시 한 명 이상의 심판이 있어야 한다.
    ...
    - 1e2) 모든 선수는 심판 업무 수행이 반드시 가능해야 한다. 업무 수행을 요구받을 시, 오직 합당한 이유(예: 퍼즐에 익숙하지 않음)로만 WCA 파견위원의 재량하에 업무로부터 제외될 수 있다. 페널티: 대회의 한 종목, 복수의 종목 또는 전 종목에서 실격([규정 2k](regulations:regulation:2k) 참고).
- 1f) 각 종목은 반드시 한 명 이상의 스크램블러가 있어야 한다. 예외: 3x3x3 최소회전.
    ...
    - 1f2) 모든 선수는 스크램블러 업무 수행이 반드시 가능해야 한다. 업무 수행을 요구받을 시, 오직 합당한 이유(예: 스크램블링 표기법에 익숙하지 않음)로만 WCA 파견위원의 재량하에 업무로부터 제외될 수 있다. 페널티: 대회의 한 종목, 복수의 종목 또는 전 종목에서 실격([규정 2k](regulations:regulation:2k) 참고).
- 1g) 각 종목은 반드시 한 명 이상의 기록 입력자가 있어야 한다.
    - 1g1) 기록 입력자는 결과를 집계하는 책임이 있다.
    - 1g2) 기록지의 결과에 대한 변경은 오직 WCA 파견위원의 재량하에만 허용된다.
- 1h) 한 종목의 동일한 라운드에 속한 선수는 하나의 조 혹은 복수의 조로 나누어져 참가할 수 있다.
    - 1h1) 한 라운드의 스크램블러와 심판은 해당 라운드에서 그들의 모든 시도가 완료되기 전에는 자신이 속한 조의 선수에 대하여 스크램블러/심판 업무를 해서는 안 된다. WCA 파견위원의 재량하에 자신이 속한 조에 대하여 스크램블 업무/심판 업무를 할 수 있지만, 운영진은 그들이 아직 측정하지 않은 시도에 대한 스크램블 시퀀스를 볼 수 없다는 것을 반드시 보장해야 한다([규정 4b2](regulations:regulation:4b2) 참고).
...
- 2e) 선수는 반드시 시민권을 보유하고 있는 지역을 대표해야 한다. WCA 파견위원은 선수의 첫 대회에서 문서(예: 여권)를 통해 시민권을 확인해야 한다. 선수가 본인이 등록한 지역을 대표할 수 없다고 판단되면, 해당 선수는 WCA 이사회의 재량하에 소급적으로 실격 및/또는 정지될 수 있다.
...
- 2g) 선수는 반드시 지정된 대회 구역 안에서 정숙을 유지해야 한다. 대화는 허용되나 적당한 수준에서 이루어져야 하며, 기록 측정 중인 선수로부터 떨어져 있어야 한다.
    - 2g3) 같은 조의 모든 선수가 더 이상 남은 시도가 없지 않은 한, 측정 진행 중인 조에 속한 선수는 같은 조에 속한 다른 선수와 절대로 해당 조의 스크램블 상태에 대하여 소통하여서는 안 된다. 페널티: WCA 파견위원의 재량하에 해당 종목으로부터 선수 실격. 예외: 3x3x3 최소회전 종목에서는 측정을 마친 선수가 관련된 시도에 대해 측정하고 있지 않은 사람과 스크램블 시퀀스를 공유할 수 있다.
- 2h) 선수는 대회장 내에 있는 동안 반드시 적절한 복장을 착용해야 한다. 선수는 WCA 파견위원의 재량하에 부적절한 복장으로 인해 대회로부터 실격될 수 있다.
- 2i) 미리보기 혹은 솔빙 도중, 선수는 스택 매트 타이머 혹은 스톱워치 외의 전자 기기 혹은 음향 기기를 절대로 사용해서는 안 된다(예: 휴대 전화, MP3 플레이어, 녹음기, 추가적인 조명). 페널티: 해당 시도의 실격(DNF).
    - 2i1) WCA 파견위원의 재량하에, 선수는 불공평한 이득을 주지 않는 비 전자 기기를 사용할 수 있다. 이것은 다음을 포함한다:
        ...
        - 2i1c) 손난로.
        - 2i1d) 식음료.
    - 2i2) 선수는 WCA 파견위원의 재량하에 측정 장소에서 카메라를 사용할 수 있으나, 시도의 시작부터 선수가 솔브를 멈출 때까지 다음과 같은 제한이 적용된다. 제한을 어길 시 페널티 : 해당 시도의 실격(DNF).
...
- 2u) 선수는 기록 측정을 위해 호출될 때 반드시 대회장에 출석해야 하며 참가할 준비가 되어야 한다. 페널티: 종목으로부터 실격.
    - 2u1) 예외: 개별적으로 예정된 시도(예: 최소회전, 3x3x3 눈 가리고 맞추기(복수))에 제시간에 출석하지 않은 선수는 WCA 파견위원의 재량하에 해당 시도를 거부한 것(DNS)으로 간주할 수 있다.
